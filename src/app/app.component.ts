...
import { InvalidPinnedPageInformationError, UnauthenticatedError } from './services/dataaccess/api';
import { PinnedPageManagementService } from './services/module/pinned-page-management/pinned-page-management.service';
import { SessionManagementService, SessionUserInfo } from './services/module/session-management';

export class AppSubmenuItem {
  constructor(public title: string, public url: string, public onClick: () => void | Promise<void>) {}
}

export class AppMenuItem {
  constructor(public title: string, public icon: string, public submenuItemList: AppSubmenuItem[]) {}
}
...
export class AppComponent {
  ...
  constructor(
    ...
    this.sessionManagementService.subscribeForSessionUserInfo((sessionUserInfo) =>
      this.onSessionUserInfoChanged(sessionUserInfo)
    );
  }

  private onSessionUserInfoChanged(sessionUserInfo: SessionUserInfo | null): void {
  ...
  private updateMenuItemList(sessionUserInfo: SessionUserInfo): void {
    ...
    if (this.sessionManagementService.checkSessionUserHasPermission('images.upload')) {
      labelSubmenuList.push(new AppSubmenuItem('Upload images', '/upload-images', () => {}));
    }
    if (this.sessionManagementService.checkSessionUserHasPermission('images.manage.self')) {
      labelSubmenuList.push(new AppSubmenuItem('My images', '/my-images', () => {}));
    }
    if (this.sessionManagementService.checkSessionUserHasPermission('images.manage.all')) {
      labelSubmenuList.push(new AppSubmenuItem('All images', '/all-images', () => {}));
    }
    if (this.sessionManagementService.checkSessionUserHasPermission('images.verify')) {
      labelSubmenuList.push(new AppSubmenuItem('Verify images', '/verify-images', () => {}));
    }
    ...
    if (this.sessionManagementService.checkSessionUserHasPermission('images.manage.all')) {
      labelSubmenuList.push(new AppSubmenuItem('Classification tasks', '/classification-tasks', () => {}))
    }
    this.menuItemList.push(new AppMenuItem('Label data', 'edit', labelSubmenuList));

    // Settings menu
    const settingsSubmenuList: AppSubmenuItem[] = [];
    settingsSubmenuList.push(new AppSubmenuItem('My pinned pages', '/pinned-pages', () => {}));
    if (this.sessionManagementService.checkSessionUserHasPermission('users.manage')) {
      settingsSubmenuList.push(new AppSubmenuItem('Manage users', '/manage-users', () => {}));
    }
    if (this.sessionManagementService.checkSessionUserHasPermission('user_roles.manage')) {
      settingsSubmenuList.push(new AppSubmenuItem('Manage user roles', '/manage-roles', () => {}));
    }
    if (this.sessionManagementService.checkSessionUserHasPermission('user_permissions.manage')) {
      settingsSubmenuList.push(new AppSubmenuItem('Manage user permissions', '/manage-permissions', () => {}));
    }
    if (this.sessionManagementService.checkSessionUserHasPermission('user_tags.manage')) {
      settingsSubmenuList.push(new AppSubmenuItem('Manage user tags', '/manage-tags', () => {}));
    }
    if (this.sessionManagementService.checkSessionUserHasPermission('image_types.manage')) {
      settingsSubmenuList.push(new AppSubmenuItem('Manage image types', '/manage-image-types', () => {}));
    }
    if (this.sessionManagementService.checkSessionUserHasPermission('image_tags.manage')) {
      settingsSubmenuList.push(new AppSubmenuItem('Manage image tags', '/manage-image-tags', () => {}));
    }
    ...
    this.menuItemList.push(new AppMenuItem('Settings', 'setting', settingsSubmenuList));
  }
  ...
  public async onPinThisPageClicked(): Promise<void> {
    ...
    try {
      await this.pinnedPageManagementService.createPinnedPage(this.router.url, this.pinPageDescription);
      ...
      if (e instanceof InvalidPinnedPageInformationError) {
        ...
        this.notificationService.error('Failed to pin page', 'User is not logged in');
        this.router.navigateByUrl('/login');
      } else {
