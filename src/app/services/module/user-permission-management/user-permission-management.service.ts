...
import { PermissionsService, RolesService, UserPermission } from '../../dataaccess/api';
...
export class UserPermissionManagementService {
  ...
  public isValidPermissionName(permissionName: string): { [k: string]: boolean } | null {
  ...
  public async createUserPermission(permissionName: string, description: string): Promise<UserPermission> {
    return this.permissionsDataAccessService.createUserPermission(permissionName.trim(), description.trim());
  }
  ...
  public async updateUserPermission(
    ...
    return this.permissionsDataAccessService.updateUserPermission(id, permissionName, description);
  }
  ...
  public async addUserPermissionToUserRole(userRoleID: number, userPermissionID: number): Promise<void> {
    await this.rolesDataAccessService.addUserPermissionToUserRole(userRoleID, userPermissionID);
  }

  public async removeUserPermissionFromUserRole(userRoleID: number, userPermissionID: number): Promise<void> {
    await this.rolesDataAccessService.removeUserPermissionFromUserRole(userRoleID, userPermissionID);
  }
}
