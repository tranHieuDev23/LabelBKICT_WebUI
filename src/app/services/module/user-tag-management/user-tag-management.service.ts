...
import { UserTagsService, UsersService, UserTag, UserTagListSortOrder } from '../../dataaccess/api';
...
export class UserTagManagementService {
  ...
  public async createUserTag(displayName: string, description: string): Promise<UserTag> {
    return await this.tagsDataAccessService.createUserTag(displayName.trim(), description.trim());
  }
  ...
  public async getUserTagList(
    ...
    return await this.tagsDataAccessService.getUserTagList(offset, limit, sortOrder);
  }

  public async updateUserTag(id: number, displayName: string, description: string): Promise<UserTag> {
    if (displayName !== undefined) displayName = displayName.trim();
    if (description !== undefined) description = description.trim();
    return await this.tagsDataAccessService.updateUserTag(id, displayName, description);
  }
