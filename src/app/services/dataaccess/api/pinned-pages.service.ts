...
export class PinnedPagesService {
  ...
  public async createPinnedPage(url: string, description: string, screenshotData: Blob): Promise<PinnedPage> {
    try {
  ...
  public async getPinnedPageList(
    ...
    try {
      ...
      const pinnedPageList = response.data.pinned_page_list.map(PinnedPage.fromJSON);
